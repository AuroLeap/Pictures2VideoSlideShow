...
"#
    ...
    fn encoder_field_defaults_validates_and_rejects_sr034() {
        ...
        // Per-output: two outputs with different encoders both validate. Names
        // must differ or the (merged) SR-017 duplicate-name check fires first.
        let a: OutputDef = toml::from_str(&output_toml("encoder = \"h264_nvenc\"")).expect("parse");
        let mut b: OutputDef =
            toml::from_str(&output_toml("encoder = \"software\"")).expect("parse");
