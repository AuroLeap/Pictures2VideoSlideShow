...
fn demo_config_loads_and_validates_sr033() {
    ...
    let mut config = Config::from_file(&cfg_path)
        .unwrap_or_else(|e| panic!("demo_config.toml should parse: {e}"));

    // The shipped media_root is RELATIVE by design — demo.bat `cd`s beside the
    // exe before running, and demo-media/ only exists after it downloads the
    // samples. Pin the shipped value, then validate against a stand-in dir so
    // the semantic checks run from a clean checkout too.
    assert_eq!(
        config.input.media_root,
        Path::new("demo-media"),
        "demo config scans the folder demo.bat downloads into"
    );
    let stand_in = std::env::temp_dir().join(format!("demo_assets_media_{}", std::process::id()));
    std::fs::create_dir_all(&stand_in).expect("create stand-in media dir");
    config.input.media_root = stand_in.clone();
    let validated = config.validate();
    std::fs::remove_dir_all(&stand_in).ok();
    validated.unwrap_or_else(|e| panic!("demo_config.toml should pass validation: {e}"));

    assert_eq!(
        config.outputs.len(),
        1,
        "demo ships exactly one frame output"
    );
    ...
    assert!(
        out.enable_audio,
        "demo enables source-video audio passthrough"
    );
